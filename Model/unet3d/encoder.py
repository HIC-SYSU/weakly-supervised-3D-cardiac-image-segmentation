...
from .basic_block import conv3x3x3, conv1x1x1

class MyronenkoConvolutionBlock(nn.Module):#卷积块
...
class MyronenkoResidualBlock(nn.Module):#残差块
    def __init__(self, in_planes, planes, stride=1, norm_layer=None, norm_groups=8, kernel_size=3):
...
class MyronenkoLayer(nn.Module):#层
...
class ConvolutionalAutoEncoder(nn.Module):
    def __init__(self, input_shape=None, n_features=1, base_width=32, encoder_blocks=None,feature_dilation=2,
                 downsampling_stride=2,encoder_class=MyronenkoEncoder, layer_widths=None,kernel_size=3):
    ...
    def forward(self, x):
        x = self.encoder(x)#一共4个[1,32,64,64,32],[1,64,32,32,16],[1,128,16,16,8],[1,256,16,16,8]
        return x
